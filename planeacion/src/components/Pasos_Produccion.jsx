...
const Pasos_Produccion = ({ setIsAuthenticated }) => {
  ...
  //     setCantidad(detalle.cantidad_terminada);
  //     setCodigo('');
  //     setShowModal(true);
  //   } else {
  ...
  return (
    <div className='container mt-5'>
      ...
      <label style={{ fontWeight: 'bold'  }}>Folio: {detalles[0]?.folio || 'Folio no disponible'}</label>

      <label style={{ marginLeft:'20px', fontWeight: 'bold' }}>Cliente: {detalles[0]?.nombre_cliente || 'Cliente no disponible'}</label>

        <label style={{ marginLeft:'20px'}}>Producto: {detalles[0]?.nombre_producto || 'Producto no disponible'}</label>
        <label style={{ marginLeft: '20px' }}>Fecha de Entrega: {detalles[0]?.fecha_entrega_formato || 'Fecha no disponible'}</label>
        <br /><br />
        <label style={{ marginLeft: '' }}>Dimensión: {detalles[0]?.dimension || 'Dimensión no disponible'}</label>
        <label style={{ marginLeft: '20px' }}>Cantidad Solicitada: {detalles[0]?.cantidad_solicitada || 'Dimensión no disponible'}</label><br /> <br />
      </div>
      <br />
        ...
        <tbody>
          {detalles.map((detalle, index) => (
            <tr key={index}>
              ...
              {/* <td>
                <button className="btn btn-success" onClick={() => handleOpenModal(detalle)}>
                  Modificar
                </button>
              </td> */}
              <td>
            ...
            <div className="modal-dialog">
              <div className="modal-content">
                <div className="modal-header" style={{ backgroundColor: '#4b4b4b' }}>
                  ...
                  <button type="button" className="btn-close" onClick={handleCloseModalHistorial} aria-label="Cerrar" style={{ color: 'white' }}></button>
                </div>

                <div className="modal-body" style={{ backgroundColor: '#4b4b4b' }}>
                  <h5>Paso: {nombrePaso}</h5>
...
export default Pasos_Produccion;
