...
const PrincipalPlaneacion = ({ setIsAuthenticated }) => {
  ...
  const handleCompletos = () => {
    navigate(`/pedido-completo-planeacion`);
  };
  ...
  const handleEliminarPedido = async (idPedido) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        const response = await axios.delete(`http://localhost:3306/eliminar-pedido/${idPedido}`, {
